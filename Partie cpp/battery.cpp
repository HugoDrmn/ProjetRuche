...
int main(int argc, char *argv[]) {
    ...
    // derniére capacité en Ah enregistrée
    ...
    if (!batterie.obtenirErreur()) {
        ...
        t1 = time(0);
        if ((t1 - t0) < 3600) {// deltaT maxi 1 heure utile aprés un long arrèt
            charge +=z*(t1-t0)/3600 + (i-z)*(t1-t0)/7200;//calcule avec la technique des trapèze
            z = i;
        }
        if (charge < 0) {
        ...
        //écriture de la nouvelle charge et de la date de la la dernière relève
